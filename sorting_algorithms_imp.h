...
void selectionSort(T arr, int n, bool is_vis)
{
    ...
    for (i = 0; i < n - 1; i++)
    {
        ...
        min_idx = i;
        visSetAttr(i, 0);
        visPrintCurrentOperation("SELECT PIVOT");
        visWait();
        for (j = i + 1; j < n; j++)
        {
            visSetAttr(j, 0);
            visPrintCurrentOperation("CHECK FOR SMALLEST ELEMENT");
            visWait();
        ...
        visPrintCurrentOperation("CHANGE PIVOT WITH SMALLEST");
        visSwap(i, min_idx);
...
void bubbleSort(T arr[], int n, bool is_vis)
{
    ...
    for (i = 0; i < n - 1; i++)
    {
        ...
        for (j = 0; j < n - i - 1; j++)
        {
            visSetAttr(j, 0);
            visSetAttr(j + 1, 0);
            visPrintCurrentOperation("CHECK IF NEXT ELEMENT SMALLER");
            visWait();
            if (arr[j] > arr[j + 1])
            {
                swap(arr[j], arr[j + 1]);
                visPrintCurrentOperation("CHANGE ELEMENTS");
                visSwap(j, j + 1);
            }
            visPrintCurrentArray();
            visWait();
...
void insertionSort(T arr[], int n, bool is_vis)
{
    ...
    for (i = 1; i < n; i++)
    {
        ...
        //visVerticalLine(i);
        visSetAttr(i, 0);
        visPrintCurrentOperation("SET PIVOT");
        visWait();
        j = i - 1;
        ...
        arr[j + 1] = key;
        visPrintCurrentOperation("REPLACE");
...
stringstream merge_string, merge_string2, merge_string3;
...
void merge(T arr[], int l, int m, int r, bool is_vis)
{
    ...
    merge_string << "MERGING SORTED ARRAYS";
    visPrintSecondCurrentOperation(merge_string.str());
    merge_string.str("");
...
void mergeSort(T arr[], int l, int r, bool is_vis)
{
    ...
    merge_string << "MERGING BETWEEN " << l << ". element and " << r << ". element";
    visPrintSecondCurrentOperation(merge_string.str());
    merge_string.str("");
...
std::string quick_string;
stringstream quick_string2;
stringstream quick_string3;
...
int partition(T arr[], int low, int high, bool is_vis)
{
    ...
    visPrintCurrentArray();
    visSetAttr(high, 0);
    quick_string2 << "PIVOT = " << arr[high];
    visPrintSecondCurrentOperation(quick_string2.str());
    quick_string2.str("");
    visWait();
    ...
    {
        quick_string2 << "PIVOT = " << arr[high] << ", COMPARING PIVOT WITH " << arr[j];
        visPrintSecondCurrentOperation(quick_string2.str());
        quick_string2.str("");
        visCrossVertical(high);
        visCrossVerticalToLeft(low);
        ...
        if (arr[j] < pivot)
        {
            ...
            if (i != j)
            {
                quick_string3 << "ITS SMALLER THAN PIVOT, INCREASE COUNT BY 1 AND REPLACE ELEMENTS";
                visPrintThirdCurrentOperation(quick_string3.str());
                quick_string3.str("");
                visRemoveLineBetween();
        ...
        visPrintThirdCurrentOperation("");
    }
    swap(arr[i + 1], arr[high]);
    visRemoveLineBetween();
    quick_string2 << "REPLACE PIVOT TO ITS PLACE IN SORTED ARRAY";
    visPrintSecondCurrentOperation(quick_string2.str());
    quick_string2.str("");
    visSwap(i + 1, high);
    visWait();
...
void quickSort(T arr[], int low, int high, bool is_vis)
{
    ...
    if (low < high)
    {
        ...
        at right place */
        quick_string = "PARTITIONING BETWEEN " + std::to_string(low) + ". element and " + std::to_string(high) + ". element";
        visPrintCurrentOperation(quick_string);
...
void heapSort(T arr[], int n, bool is_vis)
{
    ...
    for (int i = n - 1; i > 0; i--)
    {
        ...
        //visPrintCurrentOperation("HEAPIFY");
...
{
    std::string shell_operation_string = "";
    ...
    for (int gap = n / 2; gap > 0; gap /= 2)
    {
        ...
        for (int i = gap; i < n; i += 1)
        {
            ...
            for (j = i; j >= gap && arr[j - gap] > temp; j -= gap)
            {
                ...
                visSetAttr(j, 2);
                visSetAttr(j - gap, 2);
                shell_operation_string = "COMPARE VALUES, GAP=" + std::to_string(gap);
                visPrintCurrentOperation(shell_operation_string);
                visWait();
                arr[j] = arr[j - gap];
                shell_operation_string = "CHANGE VALUES, GAP=" + std::to_string(gap);
                visPrintCurrentOperation(shell_operation_string);
                visSwap(j - gap, j);
                visPrintCurrentArray();
                visRemoveLineBetween();
